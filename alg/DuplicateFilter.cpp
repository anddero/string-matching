    ...
    for (unsigned line_index = ref_line_index + 1; line_index != source_lines.size(); ++line_index) {
        ...
        for (auto& diff_dup_pair : ascii_duplicates) {
            if (similarity >= 1.f - diff_dup_pair.first) {
                if (has_obvious_number_difference(similarity_result, line.line, ref_line.line)) {
                    break;
                }
...
bool DuplicateFilter::has_obvious_number_difference(const SearchQuerySimilarityResult &similarity_result,
    ...
    int min_size = std::min(ref_line_normalized.size(), line_matched.size());

    for (unsigned i = 0; i != min_size; ++i) {
        if (ref_line_normalized[i] != line_matched[i]) {
            if (is_digit(ref_line_normalized[i]) && is_digit(line_matched[i])) return true; // possible semantic difference
        }
    }

    return false; // only if no obvious number differences (there may still be less obvious ones which must be proofread manually)
}
